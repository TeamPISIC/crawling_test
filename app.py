...
name = ['최효정','임채원','김희진']
school= '서울여대'
num = ['01068785410', '01072724557']
email = ['hyojeong113@naver.com','cbqnk9@swu.ac.kr']
...
#크롤링 코드
@server.route('/printt', methods=['GET'])
def printt():
    c_data=[]
    #1. 이름 x 이메일
    for a in range(len(name)):
        for b in range(len(email)):
            plusUrl = name[a] + " "+ email[b] # 네임, 이메일 둘 다 
            #print(plusUrl)
            #printt(plusUrl)
            baseUrl = 'https://www.google.com/search?q='
            #plusUrl = input('무엇을 검색할까요? :') 
            #two(name,email,plusUrl)
            #plusUrll = plusURL
            ...
            for i in v:
                if m == 3:
                    ...
                    p = i.select_one('.LC20lb.DKV0Md').text # 제목
                    c_data.append(p)
    ...
    return render_template('test.html', data=c_data)
